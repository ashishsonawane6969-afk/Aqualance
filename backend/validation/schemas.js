...
const geoLeadSchema = Joi.object({
  ...
  photo_data: Joi.string()
    .pattern(/^data:image\/(jpeg|jpg|png|webp|gif);base64,[A-Za-z0-9+/]+=*$/)
    // SECURITY FIX: was 27_000_000 (~27MB). Reduced to 5_000_000 (~3.75MB binary image).
    // Matches the photoBodyParser limit. Client-side canvas compression (quality=0.7)
    // keeps real shop photos well under 1MB.
    .max(5_000_000)
    .optional().allow('', null),
  notes:       safeText(1000).optional().allow('', null),
  visited_at:  Joi.string().isoDate().optional().allow('', null),
