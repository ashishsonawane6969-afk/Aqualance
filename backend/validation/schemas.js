...
/* ── PRODUCTS ─────────────────────────────────────────────────────────────── */
const _imageField = Joi.alternatives().try(
  Joi.string().uri().max(2048),
  Joi.string().pattern(/^data:image\/[a-zA-Z0-9.+_-]+;base64,[A-Za-z0-9+\/\r\n]+=*$/).max(10_000_000)
).optional().allow('', null);
...
module.exports = {
  ...
  productWriteSchema, productQuerySchema, variantBulkSchema,
  deliveryBoySchema,
